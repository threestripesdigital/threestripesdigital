...
export async function syncPostHog(env){
 ...
 try{
  ...
  const query=`SELECT toDate(toTimeZone(timestamp, 'America/New_York')) AS day, properties.campaign_id AS campaign_id, properties.adset_id AS adset_id, properties.ad_id AS ad_id, ${eventNames.map(e=>`uniqExactIf(properties.$session_id, event = 'rb_${e}') AS ${e}`).join(', ')} FROM events WHERE timestamp >= toDateTime('${start} 00:00:00') AND properties.funnel = 'rank_boost' AND properties.utm_source = 'meta' AND coalesce(toString(properties.internal), 'false') NOT IN ('true', '1') AND properties.$session_id IS NOT NULL GROUP BY day, campaign_id, adset_id, ad_id LIMIT 10000`;
