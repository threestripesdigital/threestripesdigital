...
test('PostHog import stores only aggregate rows and preserves snapshot on failure',async()=>{
 ...
 try{
  globalThis.fetch=async(url,opts)=>{assert.match(url,/605937\/query/);const q=JSON.parse(opts.body).query.query;assert.match(q,/properties.utm_source = 'meta'/);assert.match(q,/coalesce\(toString\(properties.internal\), 'false'\) NOT IN \('true', '1'\)/);assert.match(q,/uniqExactIf/);return Response.json({results:[['2026-09-12','1','2','3',2,2,1,1,0,0,1,1,0,0]]});};
