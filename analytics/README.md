...
The campaign is pinned to `120249029003230545` in the original CAD ad account `358826439854169`. All 22 ad sets and 34 ads remain paused until the owner explicitly authorizes activation. The total daily budget is CAD 206.76: 18 ad sets at CAD 9.40 and four at CAD 9.39. This is USD 150 at the fixed Bank of Canada reference rate of 1 USD = 1.3784 CAD dated September 8, 2026 (https://www.bankofcanada.ca/rates/exchange/daily-exchange-rates/). Reporting uses the account's `America/New_York` time zone and converts CAD spend to estimated USD using that dated rate, not the card settlement rate. The USD migration campaign `120251425577330371` remains paused while Meta support addresses the portfolio and billing setup. It currently has 22 ad sets and no ads; 34 creatives were prepared.
...
## V2 angle test, September 16, 2026
...
## Reporting corrections, September 20, 2026

Meta Insights imports both `actions` and `conversions`. Form opens prefer `submit_application_website`, then the legacy `offsite_conversion.fb_pixel_submit_application`, then `submit_application_total`. Overlapping aliases are not summed, and a present zero is retained. These are attributed form opens, not submitted or qualified leads. The existing 90-day refresh backfills corrected counts.

All campaign video cards and table rows now use the versioned `posthog_cohort` snapshot. It replaces the previous `posthog_daily` aggregate for reporting; legacy snapshots cannot silently populate the new cards. Wistia snapshots remain available for separate diagnostics but never replace missing campaign PostHog values.

The PostHog query groups by session before aggregating daily counts. First-session attribution and day are retained across refreshes, midnight and later event attribution changes. Sessions must contain `rb_session`, begin after the campaign's exact UTC launch timestamp, and have no internal flag or known QA campaign marker (`qa`, `qa_` prefix, or `posthog_verification`) within the imported event window. V1 uses the configured original launch; V2 uses its recorded `campaign_launch:<id>`. An unknown launch is unavailable rather than treated as prelaunch paid traffic. Only aggregate counts leave PostHog. No visitor IDs or recordings are stored by the dashboard.

Snapshots record project, reporting timezone, launch boundaries and capture window. A changed launch boundary or legacy/incompatible snapshot fails closed until reimport. Counts use the selected period by session start day. Events observed afterward stay with that session's cohort. The source capture starts September 12, 2026 and the import retains about 90 days, so partial historical coverage is labeled. Current-day data remains subject to capture and provider delay. Stale data never falls back to Wistia. The import can be refreshed through the existing authenticated Refresh data action.

`rb_vsl_load` means the player element exists in the DOM, not that it is visible or ready. Play rate divides unique playing sessions by unique player-load sessions from the same cohort. Watched-half retention and form completion use that cohort as well. Application start means first input or submit attempt, not a completed lead. Browser booking confirmations remain distinct from verified Calendly bookings. Internal traffic is captured with a flag and excluded by reporting, rather than assumed absent from provider collection. No landing-page event instrumentation changes are made by this reporting update.
