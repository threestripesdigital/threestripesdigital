...
test('booking import links verified lead and preserves manual outcomes on replay',async()=>{const e=env();e.LEADS_DB=database(false);e.LEADS_DB.raw.exec("CREATE TABLE leads(lead_ref TEXT,qualified INTEGER);CREATE TABLE calendly_invitees(invitee_uri TEXT,lead_ref TEXT,scheduled_start_at TEXT,status TEXT,created_at TEXT);");await intake({kind:'session',session_id:sid,campaign_id:'111',ad_id:'222'},e);await intake({kind:'link',session_id:sid},e,'lead-1');e.LEADS_DB.raw.exec("INSERT INTO leads VALUES('lead-1',1);INSERT INTO calendly_invitees VALUES('call1','lead-1','2026-09-01T12:00:00Z','booked','2026-08-31 10:00:00');");await syncFunnel(e);assert.equal((await e.DB.prepare('SELECT session_id FROM calls').first()).session_id,sid);e.DB.raw.exec("UPDATE calls SET status='showed',qualified=0,attendance_updated_at='2026-09-01T13:00:00Z',outcome_updated_at='2026-09-01T13:00:00Z'");await syncFunnel(e);const c=await e.DB.prepare('SELECT * FROM calls').first();assert.equal(c.status,'showed');assert.equal(c.qualified,0);assert.equal((await e.DB.prepare('SELECT COUNT(*) n FROM calls').first()).n,1);});
...
test('qualification defaults unknown and manual qualification does not block attendance sync',async()=>{
 const e=env();e.LOCAL_PREVIEW='true';e.LEADS_DB=database(false);e.LEADS_DB.raw.exec("CREATE TABLE leads(lead_ref TEXT,qualified INTEGER);CREATE TABLE calendly_invitees(invitee_uri TEXT,lead_ref TEXT,scheduled_start_at TEXT,status TEXT,created_at TEXT);INSERT INTO leads VALUES('lead-1',1);INSERT INTO calendly_invitees VALUES('call1','lead-1','2026-09-01T12:00:00Z','booked','2026-08-31 10:00:00');");
