...
function database(migrate=true){
 ...
 if(migrate)for(const file of ['0001.sql','0002.sql','0003.sql','0004.sql','0005.sql'])raw.exec(readFileSync(new URL('../../../analytics/migrations/'+file,import.meta.url),'utf8'));
...
test('qualification is automatic and cannot be overridden by attendance edits',async()=>{
 const e=env();e.LOCAL_PREVIEW='true';e.LEADS_DB=database(false);e.LEADS_DB.raw.exec("CREATE TABLE leads(lead_ref TEXT,qualified INTEGER,id INTEGER DEFAULT 1,page_url TEXT DEFAULT '');CREATE TABLE calendly_invitees(invitee_uri TEXT,lead_ref TEXT,scheduled_start_at TEXT,status TEXT,created_at TEXT,event_type_uri TEXT DEFAULT 'boost-type');INSERT INTO leads(lead_ref,qualified) VALUES('lead-1',1);INSERT INTO calendly_invitees(invitee_uri,lead_ref,scheduled_start_at,status,created_at) VALUES('call1','lead-1','2026-09-01T12:00:00Z','booked','2026-08-31 10:00:00');");
 await syncFunnel(e);let c=await e.DB.prepare('SELECT * FROM calls').first();assert.equal(c.qualified,1);assert.equal(c.keyword_qualified,1);
 ...
 e.LEADS_DB.raw.exec("UPDATE calendly_invitees SET status='no_show'");await syncFunnel(e);c=await e.DB.prepare('SELECT * FROM calls').first();assert.equal(c.status,'no_show');assert.equal(c.qualified,1);assert.equal(c.qualification_updated_at,null);assert.equal(c.attendance_updated_at,null);
});
test('show and qualified rates exclude unknowns, future and cancelled calls and flag incompleteness',async()=>{
 ...
 e.DB.raw.exec("UPDATE calls SET event_type_uri='boost-type',keyword_qualified=1");
 const r=await report(e,new URL('https://local/api/report'));assert.equal(r.totals.booked,5);assert.equal(r.totals.showed,2);assert.equal(r.totals.attendanceUnknown,1);assert.equal(r.totals.qualificationUnknown,0);
 const show=r.metrics.find(m=>m.key==='show');assert.ok(Math.abs(show.value-200/3)<1e-10);assert.equal(show.sample,3);assert.equal(show.provisional,true);
 const qualified=r.metrics.find(m=>m.key==='qualified_rate');assert.equal(qualified.value,100);assert.equal(qualified.sample,5);assert.equal(qualified.provisional,false);
 assert.equal(r.calls.length,6); // All real bookings are managed; paid costs retain their attribution requirement.
...
test('website consultations do not enter main-offer dashboard booking totals',async()=>{
 ...
 e.LEADS_DB.raw.exec("CREATE TABLE leads(lead_ref TEXT,qualified INTEGER,id INTEGER DEFAULT 1,page_url TEXT DEFAULT '');CREATE TABLE calendly_invitees(invitee_uri TEXT,lead_ref TEXT,scheduled_start_at TEXT,status TEXT,created_at TEXT,event_type_uri TEXT DEFAULT 'boost-type');INSERT INTO leads(lead_ref,qualified) VALUES('website',0),('boost',1);INSERT INTO calendly_invitees(invitee_uri,lead_ref,scheduled_start_at,status,created_at) VALUES('website-call','website','2026-09-15T12:00:00Z','booked','2026-09-08 10:00:00'),('boost-call','boost','2026-09-15T12:00:00Z','booked','2026-09-08 10:00:00');");
...
test('direct Meta bookings expose outcomes without custom browser sessions',async()=>{
 ...
 e.DB.raw.exec('UPDATE calls SET keyword_qualified=1');
 const r=await report(e,new URL('https://dashboard.test/api/report'));
 assert.equal(r.totals.showed,1);assert.equal(r.totals.qualified,1);assert.equal(r.metrics.find(m=>m.key==='show').value,100);
});

test('unattributed qualified bookings remain visible while tests, wrong routes and cancelled bookings do not inflate totals',async()=>{
 const e=env(),now=new Date().toISOString(),day=dayIn(now,e.REPORTING_TIMEZONE);e.BROWSER_TRACKING_ENABLED='false';
 e.DB.raw.prepare('INSERT INTO state VALUES(?,?)').run('funnel_success',now);
 for(const [id,source,isTest,status,type,campaign] of [['missing','',0,'scheduled','boost-type',''],['paid','meta',0,'scheduled','boost-type','111'],['test','meta',1,'scheduled','boost-type','111'],['old','meta',0,'cancelled','boost-type','111'],['other','meta',0,'scheduled','website-type','111']]){
  e.DB.raw.prepare('INSERT INTO calls(id,booked_at,booked_day,scheduled_at,scheduled_day,status,updated_at,keyword_qualified,booking_source,booking_campaign,event_type_uri,is_test) VALUES(?,?,?,?,?,?,?,1,?,?,?,?)').run(id,now,day,now,day,status,now,source,campaign,type,isTest);
 }
 e.DB.raw.prepare('INSERT INTO meta_daily VALUES(?,?,?,?,?,?,?,?,?)').run(day,'222','Ad','333','111','Rank Boost',100,1000,10);
 let r=await report(e,new URL('https://local/api/report'));
 assert.equal(r.bookingReporting.total,2);assert.equal(r.bookingReporting.qualified,2);assert.equal(r.bookingReporting.unattributed,1);assert.equal(r.metaBookings.count,1);assert.equal(r.metrics.find(m=>m.key==='qualified_cost').value,100);assert.equal(r.calls.find(c=>c.id==='missing').qualified,1);assert.ok(!r.calls.some(c=>c.id==='test'));
 r=await report(e,new URL('https://local/api/report?campaign=111'));assert.equal(r.bookingReporting.total,1);assert.ok(r.calls.some(c=>c.id==='paid'));assert.ok(!r.calls.some(c=>c.id==='missing'));
});

test('booking replay retains saved attribution, excludes internal leads and honors provider cancellations',async()=>{
 const e=env();e.CALENDLY_PAT='test';e.INTERNAL_TEST_LEAD_IDS='77';e.LEADS_DB=database(false);
 e.LEADS_DB.raw.exec("CREATE TABLE leads(id INTEGER,lead_ref TEXT,qualified INTEGER,page_url TEXT);CREATE TABLE calendly_invitees(invitee_uri TEXT,lead_ref TEXT,scheduled_start_at TEXT,status TEXT,created_at TEXT,event_type_uri TEXT);");
 const uri='https://api.calendly.com/scheduled_events/event/invitees/real',testUri='https://api.calendly.com/scheduled_events/event/invitees/test';
 e.LEADS_DB.raw.prepare('INSERT INTO leads VALUES(?,?,?,?)').run(82,'real',1,'https://example.test/?utm_source=meta&campaign_id=111&adset_id=333&ad_id=222');
 e.LEADS_DB.raw.prepare('INSERT INTO leads VALUES(?,?,?,?)').run(77,'test',1,'https://example.test/');
 for(const [id,ref] of [[uri,'real'],[testUri,'test']])e.LEADS_DB.raw.prepare('INSERT INTO calendly_invitees VALUES(?,?,?,?,?,?)').run(id,ref,'2026-09-15T21:30:00Z','booked','2026-09-14 12:47:56','boost-type');
 const old=globalThis.fetch;let requests=0;globalThis.fetch=async()=>{requests++;return Response.json({resource:{uri,tracking:{}}});};
 try{
  await syncFunnel(e);assert.equal((await e.DB.prepare("SELECT value FROM state WHERE key='funnel_error'").first()).value,'');
  let c=await e.DB.prepare('SELECT * FROM calls WHERE id=?').bind(uri).first();assert.equal(c.qualified,1);assert.equal(c.booking_source,'meta');assert.equal(c.booking_campaign,'111');assert.equal(c.booking_adset,'333');assert.equal(c.booking_ad,'222');assert.equal(requests,1);
  assert.equal((await e.DB.prepare('SELECT is_test FROM calls WHERE id=?').bind(testUri).first()).is_test,1);
  await e.DB.prepare("UPDATE calls SET status='showed',attendance_updated_at='2026-09-14T13:00:00Z' WHERE id=?").bind(uri).run();e.LEADS_DB.raw.prepare("UPDATE calendly_invitees SET status='canceled' WHERE invitee_uri=?").run(uri);
  await syncFunnel(e);c=await e.DB.prepare('SELECT * FROM calls WHERE id=?').bind(uri).first();assert.equal(c.status,'cancelled');assert.equal(requests,1);assert.equal((await e.DB.prepare('SELECT COUNT(*) n FROM calls').first()).n,2);
 }finally{globalThis.fetch=old;}
});

test('v2 prelaunch remains separate from v1 baseline and retains zero-spend inventory',async()=>{const e=env(),v1='120249029003230545',v2='120249151255100545';e.META_CAMPAIGN_IDS=v1+','+v2;const today=dayIn(Date.now(),e.REPORTING_TIMEZONE);e.DB.raw.prepare('INSERT INTO meta_daily VALUES(?,?,?,?,?,?,?,?,?)').run('2026-09-11','old','Old','old-set',v1,'Cold Test v1',100,1000,20);await e.DB.prepare('INSERT INTO state VALUES(?,?)').bind('delivery_inventory',JSON.stringify([{id:v1,name:'Cold Test v1',kind:'campaign_id'},{id:v2,name:'Cold Test v2',kind:'campaign_id'},{id:'a',name:'A distrust',campaign_id:v2,kind:'adset_id',effective_status:'PAUSED'},{id:'b',name:'B proof',campaign_id:v2,kind:'adset_id',effective_status:'PAUSED'},{id:'new',name:'New',adset_id:'a',campaign_id:v2,kind:'ad_id',effective_status:'PAUSED'}])).run();const r=await report(e,new URL('https://local/api/report?campaign='+v2));assert.equal(r.totals.spend,0);assert.equal(r.hold.day,0);assert.equal(r.hold.locked,true);assert.equal(r.adsets.length,2);assert.equal(r.ads.length,1);assert.equal(r.ads[0].delivery_paused,true);assert.equal(r.comparison.totals.spend,100);assert.equal(r.comparison.period.start,'2026-09-11');assert.equal(r.comparison.period.end,'2026-09-11');assert.equal(r.campaigns.length,2);assert.equal(r.budget,100);const old=await report(e,new URL('https://local/api/report?campaign='+v1+'&days=history'));assert.equal(old.totals.spend,100);assert.equal(old.budget,0);assert.equal(old.formOpenReporting.count,null);});
